 ...
 */
export function bucketForecast(dues: number[], now: Date, days: number): ForecastDay[] {
  const start = startOfLocalDay(now);
  const buckets: ForecastDay[] = Array.from({ length: days }, (_, i) => ({
    date: localDateKey(addDays(start, i)),
    count: 0,
  }));
  const index = new Map(buckets.map((b, i) => [b.date, i]));
  for (const due of dues) {
    const key = due < start.getTime() ? buckets[0].date : localDateKey(new Date(due));
